...
namespace Repositories
{
    ...
    public class BaseRepository<TEntity> : IRepository<TEntity> where TEntity : class
    {
        ...
        {
            Db = HttpContext.Current.Items[FieldKey.DB_CONTEXT_KEY] as DbContext;
        }
        ...
        public void Update(TEntity entity, Expression<Func<TEntity, object>>[] updateProperties)
        {
            ...
            Db.Entry<TEntity>(entity).State = EntityState.Unchanged;

            if (updateProperties != null)
            {
                foreach (var property in updateProperties)
                {
                    Db.Entry<TEntity>(entity).Property(property).IsModified = true;
                }
            }
        }
